-- ============================================================
-- V000__schema_base.sql
-- DDL base del modelo de datos PE-GOL SaaS — fuente: docs/06_MODELO_DATOS.md v1.0
-- Contenido: 15 ENUMs · 25 tablas · FK circular usuario→area · RLS
-- (20 tablas, 3 funciones public.*, 20 políticas) · Seed (3 planes +
-- SuperAdmin con hash BCrypt real) · 3 vistas.
-- NOTA DESVIACION (2026-09-20): las funciones auxiliares de RLS se crean en
-- schema `public` (public.tenant_id/user_rol/area_id) en lugar de `auth.*`
-- como indica el doc 06: Supabase restringe CREATE en schema `auth`
-- (pertenece a supabase_admin; rol postgres recibe 42501 permission denied).
-- Las políticas RLS referencian public.* — comportamiento idéntico
-- (leen request.jwt.claims). Se registra en ADR-009 al cierre de HU-045.
-- Referencias: ADR-001/002/006/007 (índices versionados V001-V004,
-- NO duplicados aquí — ver sección 7 del documento 06).
-- Autor: Jorge (Dicegsa) / @Arquitecto · Fecha: 2026-09-20
-- Creada por: @BackendDev (deploy Supabase Cloud — HU-045 P0)
-- ============================================================
...
-- Índice versionado en V001/V002 (ADR-001/002) — no duplicar
...
CREATE TABLE ciclo (
    ...
    -- Índice parcial versionado en V003/V004 (ADR-006/007)
);
...
CREATE OR REPLACE FUNCTION public.user_rol() RETURNS TEXT AS $$
    SELECT NULLIF(current_setting('request.jwt.claims', true)::json->>'rol', '');
$$ LANGUAGE SQL STABLE;

CREATE OR REPLACE FUNCTION public.area_id() RETURNS UUID AS $$
...
CREATE POLICY area_tenant_policy ON area
    USING (
        ...
        AND (
            public.user_rol() IN ('SuperAdmin', 'AdminTenant', 'Gerente')
            OR id = public.area_id()
        )
    );
...
CREATE POLICY filosofia_policy         ON filosofia         USING (tenant_id = public.tenant_id());
CREATE POLICY pilar_policy             ON pilar             USING (tenant_id = public.tenant_id());
CREATE POLICY umbral_semaforo_policy   ON umbral_semaforo   USING (tenant_id = public.tenant_id());
CREATE POLICY key_result_policy        ON key_result        USING (tenant_id = public.tenant_id());
CREATE POLICY valor_mensual_kr_policy  ON valor_mensual_kr  USING (tenant_id = public.tenant_id());
CREATE POLICY entregable_policy        ON entregable_adjunto USING (tenant_id = public.tenant_id());
CREATE POLICY historial_policy         ON historial_progreso USING (tenant_id = public.tenant_id());
CREATE POLICY proyecto_capex_policy    ON proyecto_capex    USING (tenant_id = public.tenant_id());
CREATE POLICY desembolso_capex_policy  ON desembolso_capex  USING (tenant_id = public.tenant_id());
CREATE POLICY cuenta_opex_policy       ON cuenta_opex       USING (tenant_id = public.tenant_id());
CREATE POLICY subcuenta_opex_policy    ON subcuenta_opex    USING (tenant_id = public.tenant_id());
CREATE POLICY presupuesto_opex_policy  ON presupuesto_opex  USING (tenant_id = public.tenant_id());
CREATE POLICY rubro_material_policy    ON rubro_material    USING (tenant_id = public.tenant_id());
CREATE POLICY alerta_enviada_policy    ON alerta_enviada    USING (tenant_id = public.tenant_id());
...
-- SUPER ADMIN INICIAL
-- ============================================================
-- Contraseña temporal del SuperAdmin (cambiar en el primer login — OBLIGATORIO
-- por requiere_cambio_pwd = TRUE, Flag 2 HU-045). La contraseña temporal se
-- entrega por canal seguro fuera del repositorio.
-- Hash BCrypt cost 12 generado con BCrypt.Net-Next 4.2.0, verificado con BCrypt.Verify == TRUE:
...
VALUES (
    ...
    '$2a$12$K3ZE4ipt7e6hUujaWpVG1.ZE7jK/f11GVGKjdHlO/FBMGc.oCZBWi',
